...
transformed parameters {
  ...
  vector[T] atemp = atemp_z * tauTemp;
}
model {
  ...
  // likelihood
  if (likelihood == 1){
    vector[N] yhat = mu + aEC3[EC3] + aEC4[EC4] + aEC2[EC2] + bsuperking[superking] + bfamily[family] + bgenus[genus] + bspecies[species] + aS[SUBS] + atemp[Temp];
    y ~ normal(yhat, sigma);
  }
}
