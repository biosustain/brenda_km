...
def run_enzyme_model(data_df):
    ...
    fit_prior = model.sample(data=input_prior, adapt_delta=0.999, csv_basename='./enzyme_output.csv')
    fit_prior.diagnose()
    fit_posterior = enzyme_model.sample(data=input_posterior, adapt_delta=0.999)
...
def run_ec3_model(data_df):
    ...
    input_prior = {**input_base, **{'likelihood': 0}}
    input_posterior = {**input_base, **{'likelihood': 1}}
    fit_prior = model.sample(data=input_prior, adapt_delta=0.999, csv_basename='./ec3_output.csv')
    fit_prior.diagnose()
    fit_posterior = model.sample(data=input_posterior, adapt_delta=0.999)
    fit_posterior.diagnose()
    infd_prior, infd_posterior = (
        az.from_cmdstanpy(
            fit,
...
# log_sigma_samples_prior = infd_prior.posterior['log_sigma_ec3'].to_series().unstack().pipe(sort_by_median)
# log_sigma_samples_posterior = infd_posterior.posterior['log_sigma_ec3'].to_series().unstack().pipe(sort_by_median)
# f, ax = plt.subplots(figsize=[10, 25])
# plot_forest(ax, log_sigma_samples_prior, color='tab:orange')
# plot_forest(ax, log_sigma_samples_posterior)

# ax.set_ylabel('EC3')
# ax.set_xlabel('Modelled group standard deviation (log scale)')
