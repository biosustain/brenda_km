...
parameters {
  ...
  real<lower=0> sigma;
  real<lower=0> tau;
  vector<lower=0>[N_organism] tau_epsilon;
  vector[N_predictor] epsilon;
...
transformed parameters {
  ...
  vector[N_predictor] yhat =
    mu + a_ec3[ec3] + a_ec4[ec4] + b_natural * is_natural + epsilon;
}
model {
  ...
  tau_epsilon ~ lognormal(prior_tau_epsilon[1], prior_tau_epsilon[2]);
  a_ec3 ~ student_t(4, 0, tau);
  a_ec4_free ~ student_t(4, 0, tau_ec3[ec4_to_ec3[1:N_ec4_free]]);
  epsilon ~ student_t(4, 0, tau_epsilon[organism]);
  if (likelihood){
